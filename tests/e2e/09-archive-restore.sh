...
# Archived files should be replaced with small stubs
STUB_EXISTS=$(agent_exec "test -f /tmp/sync-e2e-project/hello.txt.stub && echo 'yes' || echo 'no'")
assert_eq "$STUB_EXISTS" "yes" "hello.txt.stub exists after archive"

# Stub should be small (metadata only, not full file content)
STUB_SIZE=$(agent_exec "stat -c %s /tmp/sync-e2e-project/hello.txt.stub 2>/dev/null || echo '0'")
STUB_SIZE_NUM=$((STUB_SIZE + 0))
if [ "$STUB_SIZE_NUM" -lt 1024 ]; then
  log_pass "Stub file is small (${STUB_SIZE} bytes < 1KB)"
else
  log_fail "Stub file is too large (${STUB_SIZE} bytes, expected < 1KB)"
fi
...
assert_file_on_agent "/tmp/sync-e2e-bucket/e2e-test-project/hello.txt" "Archived hello.txt exists in remote"
assert_file_on_agent "/tmp/sync-e2e-bucket/e2e-test-project/data.txt" "Archived data.txt exists in remote"

# ---------------------------------------------------------------------------
log_section "Trigger restore operation"
# ---------------------------------------------------------------------------

RESTORE_RESPONSE=$(api_post "projects/${PROJECT_ID}/restore" '{"files": ["hello.txt"]}')
...
STUB_GONE=$(agent_exec "test -f /tmp/sync-e2e-project/hello.txt.stub && echo 'exists' || echo 'gone'")
