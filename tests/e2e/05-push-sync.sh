...
# ---------------------------------------------------------------------------
log_section "Verify test project files exist on agent"
# ---------------------------------------------------------------------------
...
HELLO_CONTENT=$(agent_exec "cat /tmp/sync-e2e-bucket/e2e-test-project/hello.txt 2>/dev/null || echo ''")
assert_eq "$HELLO_CONTENT" "Hello, world!" "hello.txt synced to remote bucket"

DATA_CONTENT=$(agent_exec "cat /tmp/sync-e2e-bucket/e2e-test-project/data.txt 2>/dev/null || echo ''")
assert_eq "$DATA_CONTENT" "Test data for sync" "data.txt synced to remote bucket"

NESTED_CONTENT=$(agent_exec "cat /tmp/sync-e2e-bucket/e2e-test-project/subdir/nested.txt 2>/dev/null || echo ''")
...
HISTORY_COUNT=$(echo "$HISTORY" | jq -r '.history | length' 2>/dev/null || echo "0")
...
LAST_STATUS=$(echo "$HISTORY" | jq -r '.history[0].status' 2>/dev/null || echo "")
