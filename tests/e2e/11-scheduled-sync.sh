...
SCHEDULE_CONFIG='{"schedule": "* * * * *", "syncDirection": "push"}'
...
# Wait up to 90s for the cron job to fire (should fire within 60s)
ELAPSED=0
TIMEOUT=90
...
while [ "$ELAPSED" -lt "$TIMEOUT" ]; do
  ...
  if [ "$COUNT_NOW" -gt "$COUNT_BEFORE" ]; then
    LAST_TRIGGER=$(echo "$HISTORY_NOW" | jq -r '.history[0].trigger // empty' 2>/dev/null || echo "")
...
CONTENT=$(agent_exec "cat /tmp/sync-e2e-bucket/e2e-test-project/scheduled-test.txt 2>/dev/null || echo ''")
