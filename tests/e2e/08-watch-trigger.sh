...
RESPONSE=$(api_patch "projects/${PROJECT_ID}" '{"watchEnabled": true, "syncDirection": "push"}')
...
HISTORY_BEFORE=$(api_get "projects/${PROJECT_ID}/history")
COUNT_BEFORE=$(echo "$HISTORY_BEFORE" | jq -r '.history | length' 2>/dev/null || echo "0")
...
while [ "$ELAPSED" -lt "$TIMEOUT" ]; do
  HISTORY_NOW=$(api_get "projects/${PROJECT_ID}/history")
  COUNT_NOW=$(echo "$HISTORY_NOW" | jq -r '.history | length' 2>/dev/null || echo "0")
...
CONTENT=$(agent_exec "cat /tmp/sync-e2e-bucket/e2e-test-project/watch-test.txt 2>/dev/null || echo ''")
...
HISTORY_FINAL=$(api_get "projects/${PROJECT_ID}/history")
LAST_TRIGGER=$(echo "$HISTORY_FINAL" | jq -r '.history[0].trigger // empty' 2>/dev/null || echo "")
